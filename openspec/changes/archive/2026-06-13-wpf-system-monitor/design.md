...
- 事件到灯效编号的映射由外部 hook 脚本（`hooks/send-hook.ps1`）完成，应用本身不硬编码事件映射

**Hook 事件映射示例**（配置在 `hooks/claude-code-settings-example.json`）:

| Claude Code 事件 | ledMode | 灯效 |
|------------------|---------|------|
| PreToolUse | 2 | 绿闪 |
...
- JSON 格式简单，hook 脚本只需发送 `{"ledMode": N}` 到管道
- 映射逻辑外置到 hook 配置，应用只需接收数字编号，职责单一
...
```json
{
  ...
  "positionPresets": [
    { "name": "左上角", "left": 0, "top": 0 }
  ]
}
```

参数说明：
- `windowLeft` / `windowTop`: 窗口坐标，-1 表示自动定位
- `windowTopMost`: 窗口是否置顶
- `pollIntervalMs`: HWiNFO 采集间隔（毫秒）
...
- `positionPresets`: 窗口位置预设列表，可通过托盘菜单快速切换
